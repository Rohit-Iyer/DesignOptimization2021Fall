...
f = @(x) x(1)^2+ (x(2)-3)^2; % Updated objective function
% In the same way, we also provide the gradient of the 
% objective:
df =@(x) [2*x(1), 2*x(2)-6]; % Updated gradient

g =@(x) [x(2)^2-2*x(1); (x(2)-1)^2+5*x(1)-15]; % Adding constraints
...
for i = 1:length(solution.x)
    sol(i) = f(solution.x(:, i));  % Store all f(x1, x2) values here
    z = g(solution.x(:, i));  % Store all g(x1, x2) values here
    con1(i) = z(1);  % Store g1 constraint here
    con2(i) = z(2);  % Store g2 constraint here
end

count = 1:length(solution.x); % Iteration through each x1 and x2
...
plot(count, con2)
grid on
legend('f(x) value', 'g1(x)', 'g2(x)')
title('f(x), g1(x) and g2(x) vs. Iterations')
...
disp("Values of x1 & x2 = ");
disp(solution.x(:, end));  % The optimum x1 and x2

disp("Funcrion value (F) = ");
disp(sol(end));  % The objective function at x1 and x2

disp("g1(x1, x2) = ");
disp(con1(end));  % The first constraint function at x1 and x2

disp("g2(x1, x2) = ");
disp(con2(end));  % The second constraint function at x1 and x2
